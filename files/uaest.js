...
}(function ()
{
	...
	function a (f)
	{
		...
		{
			var n = k[m];
			return typeof n === "string" ? n : "\\u" + ("0000" + m.charCodeAt(0)
					.toString(16))
				.slice(-4)
		}) + '"' : '"' + f + '"'
	}
	...
	if (typeof JSON2.parse !== "function")
	{
		JSON2.parse = function (o, f)
		{
			...
			if (c.test(o))
			{
				...
				{
					return "\\u" + ("0000" + p.charCodeAt(0)
							.toString(16))
						.slice(-4)
				})
			}
			if ((new RegExp("^[\\],:{}\\s]*$"))
				.test(o.replace(new RegExp('\\\\(?:["\\\\/bfnrt]|u[0-9a-fA-F]{4})', "g"), "@")
	...
	Piwik = (function ()
	{
		...
		function y (an, aO)
		{
			...
			function aw (bs, bQ, bR, bt)
			{
				...
				ae(bP, bG, bv, bA, bz, s(bt) && String(bt)
					.length ? bt : bJ);
				a3(bB, "*", al, bm, af);
				bs += I(bR);
				if (aQ.length)
				...
				if (a1)
				{
					for (bz in a1)
					{
						if (Object.prototype.hasOwnProperty.call(a1, bz))
						{
							...
							if (!s(a1[bz][3]) || String(a1[bz][3])
								.length === 0)
							{
								a1[bz][3] = 0
							}
							if (!s(a1[bz][4]) || String(a1[bz][4])
								.length === 0)
							{
								a1[bz][4] = 1
							}
			...
			function bf (br, bq)
			{
				if (br !== "")
				{
					return br + bq.charAt(0)
						.toUpperCase() + bq.slice(1)
				}
				return bq
			}
			...
			function bj ()
			{
				var br, bs, bt = {
					...
					bq = (new RegExp("Mac OS X.*Safari/"))
					.test(d.userAgent) ? B.devicePixelRatio || 1 : 1;
				if (!((new RegExp("MSIE"))
					.test(d.userAgent)))
				{
